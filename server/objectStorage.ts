...
export class ObjectStorageService {
  ...
  // Gets the upload URL for an object entity (generic upload)
  ...
  async getProfileImageUploadURL(userId: string): Promise<string> {
    ...
    const objectId = `${userId}-${Date.now()}`;
  ...
  async getProfileImageFile(objectPath: string): Promise<File> {
    ...
    const imageName = parts.slice(2).join("/");
    let entityDir = this.getPrivateObjectDir();
    if (!entityDir.endsWith("/")) {
      entityDir = `${entityDir}/`;
    }
    const objectImagePath = `${entityDir}${imageName}`;
  ...
  normalizeProfileImagePath(rawPath: string): string {
    ...
    const imagePath = rawObjectPath.slice(objectEntityDir.length);
